...
bookRoutes.get('/viewbook/:id',[check("id").isMongoId().withMessage("Invalid user ID format")], getSingleBook)
...
bookRoutes.post('/addbook',[ check("image").trim().notEmpty().withMessage("Image URL is required").isURL().withMessage("Image must be a valid URL"),
    ...
    .isLength({ min: 10 }).withMessage("Excerpt must be at least 10 characters"),

  check("page_count")
    .notEmpty().withMessage("Page count is required")
    .isInt({ min: 1 }).withMessage("Page count must be a positive number"),
  ...
  check("author")
    ...
    .isLength({ min: 3 }).withMessage("Author name must be at least 3 characters"),
...
bookRoutes.patch('/deletetbook/:id',[check("id").isMongoId().withMessage("Invalid user ID format")], deleteBook)
...
check("genre").optional().trim().notEmpty().withMessage("Genre cannot be empty"),
  ...
  check("category").optional().trim().notEmpty().withMessage("Category cannot be empty").isIn([
      "Academic",
      "Fiction",
      "Non-Fiction",
      "Comics",
...
bookRoutes.get('/adminbooks', [
     check("page").optional().isInt({ min: 1 }).withMessage("Page must be a positive number"),
     check("limit").optional().isInt({ min: 1, max: 100 }).withMessage("Limit must be between 1 and 100"),
     check("search").optional().trim().isString().withMessage("Search must be a string"),
],getBooksOfSingleUser)
